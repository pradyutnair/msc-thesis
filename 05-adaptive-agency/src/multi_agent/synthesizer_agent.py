...
from multi_agent.answer_utils import extract_answer, is_refusal, normalize_answer
from multi_agent.autonomous_agent import AutonomousAgent
from multi_agent.blackboard import Blackboard
from multi_agent.types import SubQuestionStatus
...
class SynthesizerAgent(AutonomousAgent):
    ...
    async def act(self, observation: dict[str, Any], blackboard: Blackboard) -> int:
        ...
        if self.enable_consistency_check and self._consistency_template and answer:
            answer, cons_tokens = await self._consistency_check(
                question, answer, evidence_blocks, expected_answer,
            )
    ...
    async def _consistency_check(
        ...
        prompt = self._consistency_template.format(
            question=question,
            expected_answer=expected_answer or "a short factual answer",
            answer=answer,
            evidence_blocks=evidence_blocks,
        )
        messages = [{"role": "user", "content": prompt}]
