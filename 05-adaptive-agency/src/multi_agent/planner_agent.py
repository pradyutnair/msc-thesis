"""PlannerAgent: decompose + mode selection -> monitor -> signal synthesis.

Extends M6's planner with per-sub-question retrieval mode assignment.
The decomposer prompt now outputs a "mode" field per sub-question,
parsed into RetrievalMode enum values.
"""
...
_DECOMPOSITION_SCHEMA = DecompositionResult.model_json_schema()


class PlannerAgent(AutonomousAgent):
    """Decompose + assign modes -> monitor -> signal synthesis agent."""
    ...
    def __init__(
        ...
        self.last_decomposition_text: str = ""
    ...
    async def decompose_first(self, blackboard: Blackboard) -> int:
        ...
        Returns the number of sub-questions so the pipeline can create
    ...
    async def _decompose(
        ...
        for attempt in range(self.max_parse_retries):
            response = await self._async_chat(
                ...
                guided_json=_DECOMPOSITION_SCHEMA,
            )
            total_tokens += int(response.get("cost", 0.0) * 1_000_000)
            raw = response["message"].get("content", "")

            try:
                decomposition = self._parse_decomposition(raw)
            except (json.JSONDecodeError, KeyError, ValueError, Exception) as exc:
                logger.warning("Planner parse error (attempt %d/%d): %s",
                               attempt + 1, self.max_parse_retries, exc)
                continue

            sub_questions = self._to_m6_sub_questions(decomposition)
            self.last_decomposition_text = raw

            if is_redecomposition:
                await blackboard.reset_search_plan(sub_questions, preserve_verified=True)
            else:
                await blackboard.set_search_plan(sub_questions)
                blackboard.expected_answer = decomposition.expected_answer
                blackboard.question_type = decomposition.question_type

            mode_summary = {}
            for sq in sub_questions:
                mode_summary[sq.mode.value] = mode_summary.get(sq.mode.value, 0) + 1

            logger.info(
                "Planner: %sdecomposed '%s' -> %d sub-questions (type=%s, expected=%s, modes=%s)",
                "re-" if is_redecomposition else "",
                question[:60], len(sub_questions),
                decomposition.question_type, decomposition.expected_answer,
                mode_summary,
            )
        ...
        fallback = [M6SubQuestion(
            id=0, text=question, dependencies=[],
        ...
        if is_redecomposition:
            ...
            blackboard.expected_answer = "a short factual answer"
        return int(total_tokens)
...
Question: {question}"""
    ...
    def _parse_decomposition(self, raw: str) -> DecompositionResult:
        """Parse + validate LLM output into DecompositionResult via Pydantic."""
        raw_clean = raw.strip()
        ...
        self._question_type = result.question_type
        self._expected_answer = result.expected_answer
        return result

    @staticmethod
    def _to_m6_sub_questions(decomposition: DecompositionResult) -> list[M6SubQuestion]:
        """Convert validated DecompositionResult into M6SubQuestion dataclasses."""
        sub_questions = []
        for sq_data in decomposition.sub_questions:
            sq = M6SubQuestion(
                id=sq_data.index,
                text=sq_data.text,
                dependencies=list(sq_data.depends_on),
                known_entities=list(sq_data.known_entities),
                unknown_entities=list(sq_data.unknown_entities),
                search_hints=list(sq_data.search_hints),
                search_queries=list(sq_data.search_queries),
                mode=RetrievalMode(sq_data.mode),
            )
            sub_questions.append(sq)
        PlannerAgent._validate_dag(sub_questions)
