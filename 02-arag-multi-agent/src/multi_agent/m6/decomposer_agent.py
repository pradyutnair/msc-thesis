...
Acts exactly once (tick 1). Single LLM call with JSON output. Enhanced prompt
extracts known_entities/unknown_entities and question type. Falls back to a
single sub-question on parse failure.
"""
    ...
    async def act(self, observation: dict[str, Any], blackboard: Blackboard) -> int:
        ...
        for attempt in range(self.max_parse_retries):
            try:
                ...
                total_tokens += response.get("cost", 0.0) * 1_000_000  # approx
