    ...
    async def _write_trajectory_to_cache(self, agent_index: int, trajectory: list[dict]) -> list[str]:
        ...
        for entry in trajectory:
            ...
            chunk_ids = entry.get("arguments", {}).get(
                "chunk_ids",
                entry.get("arguments", {}).get("chunk_id", []),
            )
    ...
    async def run(self, sub_question: SubQuestion, resolved_answers: dict[int, str] | None = None) -> AgentResult:
        ...
        try:
            result = await loop.run_in_executor(
                None,
                lambda: agent.run(sub_question.text, context=context),
            )
        except Exception as exc:
        ...
        evidence_ids = await self._write_trajectory_to_cache(
            sub_question.index,
        ...
        if supporting:
            ...
            for cid in supporting:
                if cid not in seen:
                    ...
                    seen.add(cid)
        logger.info(
            "Agent %d: '%s' → '%s' (%d loops, %.1fs, conf=%.2f)",
            sub_question.index,
