#!/usr/bin/env python3
"""
Strict evaluation script — judges ONLY the final answer claim, not reasoning.

Identical to eval.py except for the judge prompt, which explicitly instructs
the LLM to ignore explanations/reasoning and evaluate only the core answer.
...
def strip_reasoning_tags(text):
    ...
    if not isinstance(text, str):
        text = str(text)
    cleaned = REASONING_TAG_BLOCK_RE.sub("", text)
    cleaned = REASONING_TAG_OPEN_RE.sub("", cleaned)
    cleaned = REASONING_TAG_CLOSE_RE.sub("", cleaned)
...
class Evaluator:
    ...
    def calculate_llm_accuracy(self, pred_answer, gold_answer):
        """Use LLM to judge if prediction is correct — strict final-answer-only prompt."""
        system_prompt = "You are an expert evaluator. You must evaluate ONLY the final answer, not any reasoning or explanation."
        user_prompt = f"""Evaluate whether the generated answer is correct by comparing it with the gold answer.

CRITICAL INSTRUCTIONS:
- The generated answer may contain explanations, reasoning, or multiple statements.
- You must identify the FINAL ANSWER CLAIM (the specific entity, name, date, number, or yes/no that answers the question) and evaluate ONLY that.
- Ignore all reasoning, hedging, caveats, or alternative possibilities mentioned in the text.
- If the generated answer mentions the correct answer somewhere but its main/final claim is different, mark it INCORRECT.
- If the generated answer is verbose but its core claim matches the gold answer, mark it CORRECT.

Generated answer: {pred_answer}
Gold answer: {gold_answer}

Equivalence rules (all count as correct):
- Common name = full legal name (e.g., "Jean Cocteau" = "Jean Maurice Eugene Clement Cocteau")
...
- Number words = digits (e.g., "eight" = "8")
...
Response:"""
    ...
    def evaluate_single(self, idx, prediction):
        ...
        pred_answer = strip_reasoning_tags(pred_answer)
    ...
    def evaluate(self, max_workers, output_dir=None):
        ...
        logger.info(f"  LLM Accuracy (strict): {llm_accuracy:.4f}")
...
def main():
    parser = argparse.ArgumentParser(description="Evaluate ARAG predictions (strict judge)")
    parser.add_argument("--predictions", "-p", required=True, help="Predictions file path")
    ...
    print(f"\n{'='*60}")
    print(f"ARAG Strict Evaluation")
    print(f"{'='*60}")
