...
Usage:
    python scripts/m6_runner.py \
        ...
        --dataset hotpotqa \
        --output results/m6/hotpotqa/full \
        --limit 1000 --workers 5
"""
...
def build_tools(config: Config) -> ToolRegistry:
    """Build tool registry from config (same as M5/E2)."""
...
def result_to_prediction(item: dict[str, Any], result: M6PipelineResult) -> dict[str, Any]:
    """Serialize M6PipelineResult to prediction dict for JSONL output."""
...
class M6BatchRunner:
    """Async batch runner with checkpoint resume for M6 pipeline."""
...
def main():
    ...
    parser = argparse.ArgumentParser(description="M6 Blackboard-Coordinated Multi-Agent Runner")
