"""Run Standard RAG with FlashRAG — split retrieval/generation phases.

Phase 1: Retrieval with FAISS using full CPU parallelism (before vLLM).
...
def main():
    ...
    # ── Phase 1: Retrieval (full CPU parallelism, no vLLM yet) ──
    import faiss
    ...
    # Free retriever memory (index is ~61GB)
    del retriever
    import gc; gc.collect()
    print("Retriever freed from memory")

    # ── Phase 2: Generation (vLLM takes over GPU + sets threads=1, that's fine) ──
    ...
    print(f"\nRetrieval: {t_retrieval:.1f}s | Generation+Eval: {t_gen:.1f}s | Total: {t_total:.1f}s")
