...
class BaseAgent:
    ...
    def _force_finish(self, messages: List[Dict[str, Any]], context: AgentContext,
        ...
        try:
            ...
            logger.warning("BaseAgent force_finish error: %s", e)
            if self.verbose:
    ...
    def run(self, query: str, context: Optional[AgentContext] = None) -> Dict[str, Any]:
        ...
        for loop_idx in range(self.max_loops):
            ...
            try:
                ...
                logger.warning("BaseAgent sync LLM error: %s", e)
                if self.verbose:
                    print(f"LLM error: {e}")
                return {
                    "answer": "",
            ...
            # Preserve reasoning only for plain assistant turns.
            # vLLM/Qwen tool-call replay is brittle when an assistant message with
            # tool_calls is mutated into a normal content-bearing message.
        ...
        async with aiohttp.ClientSession(timeout=timeout) as session:
            for loop_idx in range(self.max_loops):
                ...
                try:
                    ...
                    logger.warning("BaseAgent async LLM error: %s", e)
                    if self.verbose:
                        print(f"LLM error: {e}")
                    return {
                        "answer": "",
    ...
    async def _async_force_finish(self, messages, context, total_cost, tool_schemas, session):
        ...
        try:
            ...
            logger.warning("BaseAgent async force_finish error: %s", e)
            return "", total_cost
