...
def v4_trajectory_decode(model, tokenizer, old_context, new_context, question,
    ...
    for step in range(steps):
        ...
        if current_agg is not None:
            current_agg[chosen] = guided[topk]
...
def main():
    ...
    for qi, q in enumerate(questions):
        ...
        for r in range(args.max_rounds):
            bc = extract_bridges_from_answer(model, tokenizer, ipool_ctx, qtext, prev_ans, args.n_candidates) if r > 0 else extract_candidates(model, tokenizer, ipool_ctx, qtext, args.n_candidates)
        ...
        for r in range(args.max_rounds):
            bc = extract_bridges_from_answer(model, tokenizer, ispread_ctx, qtext, prev_ispread_ans, args.n_candidates) if r > 0 else extract_candidates(model, tokenizer, ispread_ctx, qtext, args.n_candidates)
        ...
        for r in range(args.max_rounds):
            bc = extract_bridges_from_answer(model, tokenizer, iaram_ctx, qtext, prev_iaram_ans, args.n_candidates) if r > 0 else extract_candidates(model, tokenizer, iaram_ctx, qtext, args.n_candidates)
        ...
        v4_agg_logits = _c0_logits.clone().float()  # Start trajectory from C0
        v4_base_logits = _c0_logits.clone().float()  # Anchor for contradiction detection
        prev_v4_ans = seed_ans
        v4_round_stats = []

        for r in range(args.max_rounds):
            bc = extract_bridges_from_answer(model, tokenizer, v4_prev_ctx, qtext, prev_v4_ans, args.n_candidates) if r > 0 else extract_candidates(model, tokenizer, v4_prev_ctx, qtext, args.n_candidates)
        ...
        _all_ans = {}
        for _name, _var in [("baseline", "baseline_ans"), ("pool", "pool_ans"),
                            ("ipool", "ipool_ans"), ("ispread", "ispread_ans"),
                            ("iaram", "iaram_ans"), ("v4_trajectory", "v4_ans")]:
            if _name in methods and _var in dir():
                _all_ans[_name] = eval(_var)
        for method, ans in _all_ans.items():
