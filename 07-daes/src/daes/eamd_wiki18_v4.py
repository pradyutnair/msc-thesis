...
QUESTIONS_JSON = "/projects/prjs1800/msc-thesis/01-arag-reproduction/data/questions_wiki18/musique.json"
...
class Wiki18Retriever:
    ...
    def retrieve_batch(self, queries: list[str], top_k: int) -> list[list[str]]:
        vecs = self.model.encode(
            ...
            batch_size=min(32, max(1, len(queries))),
        ).astype(np.float32)
...
def expand_evidence(retriever: Wiki18Retriever, question: str, initial_context: str, initial_passages: list[str],
    ...
    if queries:
        for hits in retriever.retrieve_batch(queries, top_k=3):
...
def main():
    ...
    parser.add_argument("--questions_file", default=QUESTIONS_JSON)
    parser.add_argument("--output", default="/projects/prjs1800/msc-thesis/07-daes/results/eamd_wiki18_v4_50q.json")
    parser.add_argument("--run_remask", action="store_true")
    args = parser.parse_args()

    torch.backends.cuda.matmul.allow_tf32 = True
    torch.backends.cudnn.allow_tf32 = True

    retriever = Wiki18Retriever(device="cuda:0")
    ...
    questions = json.load(open(args.questions_file))[args.start_idx:args.start_idx + args.n_questions]

    methods = ["baseline", "spread", "aram", "pool", "eamd_regen"]
    if args.run_remask:
        methods.append("eamd_remask")
