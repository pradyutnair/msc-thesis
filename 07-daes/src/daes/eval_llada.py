"""
LLaDA-8B evaluation for multi-hop QA.
Uses LLaDA's native generation (no AR-shift needed — LLaDA predicts position i directly).
"""
...
def generate_llada(model, tokenizer, context, question, steps=128, n_tokens=512, temperature=0.1):
    """Generate with LLaDA. Key difference from Dream: NO AR-shift needed."""
    device = model.device
    mask_id = 126336  # LLaDA mask token

    # Few-shot prompt for concise answers
    prompt = ("You are a helpful assistant. Give a direct, concise answer (1-10 words). "
              "No explanations.\n\n"
    ...
    for step in range(steps):
        ...
        # Confidence + sampling at mask positions
...
def main():
    ...
    model = AutoModel.from_pretrained(args.model_name, trust_remote_code=True, torch_dtype=torch.bfloat16)
    ...
    print(f"Model loaded ({tag}). mask_id=126336", flush=True)

    qs = json.load(open(f"/projects/prjs1800/external/arag/data/{args.dataset}/questions.json"))[:args.n_questions]

    out_path = os.path.join(args.output_dir, f"llada_{tag}_{args.dataset}_{args.n_questions}q.jsonl")
    ...
    for i, q in enumerate(qs):
        ...
        context = "\n\n".join(passages)
        t0 = time.time()
        answer = generate_llada(model, tokenizer, context, q["question"], steps=args.steps, n_tokens=args.n_tokens)
        ...
        print(f"[{i+1}/{len(qs)}] ({elapsed:.1f}s) F1={f:.2f} contain={contain} words={len(answer.split())}", flush=True)
        print(f"  Gold: {q['answer']}", flush=True)
        print(f"  Pred: {answer[:100]}", flush=True)

    n = len(qs)
    cn = sum(1 for line in open(out_path) if json.loads(line)["contain"])
    print(f"\n{'='*60}", flush=True)
    print(f"LLaDA EVAL | {tag} | {args.dataset} | N={n}", flush=True)
