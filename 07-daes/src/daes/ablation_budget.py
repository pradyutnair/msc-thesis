"""Critical ablation: is branch-verify just retrieving more passages?
Single mode per run. Use --mode baseline_5 / baseline_14 / branch_verify."""
...
def extract_candidates(context, question, n=3):
    prompt = context + "\n\nQuestion: " + question + "\n\nThe answer is:"
    messages = [{"role": "user", "content": prompt}]
...
parser.add_argument("--mode", required=True, choices=["baseline_5", "baseline_14", "branch_verify"])
...
for i, q in enumerate(qs):
    ...
    elif args.mode == "baseline_14":
        answer = generate("\n\n".join(retrieve(q["question"], top_k=14)), q["question"])
    elif args.mode == "branch_verify":
        passages = retrieve(q["question"], top_k=5)
        context = "\n\n".join(passages)
        cands = extract_candidates(context, q["question"])
        if not cands:
            answer = generate(context, q["question"])
        else:
            best_answer, best_score = "", -1
            for c in cands:
                hop2 = retrieve(q["question"] + " " + c["text"], top_k=3)
                expanded = context + "\n\n" + "\n\n".join(hop2)
                ans = generate(expanded, q["question"])
                # Score by generation confidence
                prompt = expanded + "\n\nQuestion: " + q["question"] + "\n\nAnswer:"
                messages = [{"role": "user", "content": prompt}]
                inputs = tokenizer.apply_chat_template(messages, add_generation_prompt=True, tokenize=True)
                with torch.no_grad():
                    out = model(torch.tensor([inputs], device=model.device))
                conf = torch.softmax(out.logits[0, -1], dim=-1).max().item()
                score = conf + 0.5 * (conf - c["init_conf"])
                if score > best_score:
    ...
    print("[%d/50] (%.1fs) F1=%.2f %s" % (i+1, elapsed, sc, answer[:50]), flush=True)

print("\n%s: F1=%.1f%% Contain=%d/50=%.1f%%" % (args.mode, sf/50*100, cn, cn/50*100), flush=True)
