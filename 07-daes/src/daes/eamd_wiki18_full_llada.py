...
def eamd_remask_shared(model, tokenizer, question: str, old_context: str, new_context: str,
    ...
    old_pos = torch.tensor([n_prefix + pos for pos in positions], dtype=torch.long, device=device)
    new_pos = torch.tensor([n_prefix + pos for pos in positions], dtype=torch.long, device=device)
...
def main():
    ...
    parser.add_argument("--model_type", default="dream", choices=["dream", "llada"])
    ...
    MODEL_TYPE_REF = args.model_type
    ...
    for idx, q in enumerate(questions, start=1):
        ...
        need_aram_seed = any(name in methods for name in ("aram", "eamd_regen", "eamd_remask"))
        aram_answer = None
        aram_tokens = None
        aram_stats = None
        if need_aram_seed:
    ...
    if output_path is None:
        output_root = DEFAULT_OUTPUT_ROOT if args.model_type == "dream" else f"{DEFAULT_OUTPUT_ROOT}_llada"
        output_path = (
            f"{output_root}/{args.dataset}/shards/"
